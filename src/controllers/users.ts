...
const SQL_CREATE_USER = `INSERT INTO Users (name, email, password, salt, createdAt) VALUES (?,?,?,?,?)`;
const SQL_GET_USERS = `SELECT id,name,email,createdAt,updatedAt FROM users`;
const SQL_GET_ONE_USER = `SELECT id,name,email,password,createdAt,updatedAt FROM users WHERE id = ?`;
const SQL_GET_ONE_USER_BY_EMAIL = `SELECT id,name,email,createdAt,updatedAt FROM users WHERE email = ?`;
const SQL_UPDATE_USER = `UPDATE users SET name = ?, email = ? WHERE id = ?`;
    ...
    register : (user: userType, callback: (id?: number) => void) => {
            database.get(SQL_GET_ONE_USER_BY_EMAIL, user.email, (_err, row) => {
                if(row){
                    ...
                    const params = [user.name,user.email,bcrypt.hashSync(user.password,salt,Date.now())]
